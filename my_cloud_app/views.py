...
from .utils import create_response, create_file_response
...
def logout_user(request):
    if request.method == 'POST':
        try:
            logger.info(f"User {request.user} is trying to log out")
            request.user.auth_token.delete()
            logger.info("Выход пользователя осуществлен успешно")
            return create_response(status.HTTP_200_OK,
                                   'Выход пользователя осуществлен успешно',
                                   True)
        except Exception as e:
            logger.error(str(e))
            return Response(create_response(status.HTTP_500_INTERNAL_SERVER_ERROR, str(e)))
...
# API для работы с файловым хранилищем
